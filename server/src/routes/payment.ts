...
import { PaymentAuditService } from '../services/paymentAudit';
...
const initiateLimiter = rateLimit({
    ...
    legacyHeaders: false,
    handler: (req, res) => {
        PaymentAuditService.log(req.body.bookingId, 'INITIATE_FAILED', 'Rate Limit Exceeded', req.ip || 'unknown');
...
router.post('/initiate', initiateLimiter, async (req: AuthenticatedRequest, res: Response) => {
    const { bookingId, userDetails } = req.body;
    const clientIp = req.ip || 'unknown';

    PaymentAuditService.log(bookingId, 'INITIATE_ATTEMPT', { userDetails }, clientIp);
    ...
    try {
        ...
        */
        ...
        if (booking.paymentStatus === 'PAID') {
            PaymentAuditService.log(bookingId, 'INITIATE_FAILED', 'Already Paid', clientIp);
        ...
        const response = await fetch(paytabsUrl, {
            method: 'POST',
            headers: {
        ...
        if (data.redirect_url) {
            PaymentAuditService.log(bookingId, 'INITIATE_SUCCESS', { tran_ref: data.tran_ref }, clientIp);
        ...
        PaymentAuditService.log(bookingId, 'INITIATE_FAILED', error.message, clientIp);
 ...
 */
const verifyPayTabsSignature = (payload: any, signature: string): boolean => {
    if (!SERVER_KEY) return true; // Dev mode fallback
...
router.post('/callback', async (req: Request, res: Response) => {
    ...
    try {
        ...
        PaymentAuditService.log(cart_id, 'CALLBACK_RECEIVED', { tran_ref, status: payment_result?.response_status }, clientIp);
        ...
        if (responseStatus === 'A') {
            // Authorized
            PaymentAuditService.log(bookingId, 'VERIFICATION_SUCCESS', 'Authorized', clientIp);
            newPaymentStatus = 'PAID';
            newBookingStatus = 'CONFIRMED';
        } else if (responseStatus === 'D') {
            newPaymentStatus = 'UNPAID';
            PaymentAuditService.log(bookingId, 'VERIFICATION_FAILED', 'Declined', clientIp);
        ...
        if (newPaymentStatus === 'PAID' && updatedBooking.couponId) {
            try {
                ...
                console.log(`🎟️ Coupon ${updatedBooking.coupon?.code} usage incremented.`);
            } catch (couponErr) {
        ...
        if (newPaymentStatus === 'PAID') {
            try {
                ...
                if (updatedBooking.extraBedCount > 0 && updatedBooking.room?.hotelId) {
        ...
        if (newPaymentStatus === 'REFUNDED' && booking.paymentStatus === 'PAID') {
            try {
                ...
                if (booking.extraBedCount > 0) {
                    const room = await prisma.room.findUnique({ where: { id: booking.roomId }, select: { hotelId: true } });
                    if (room) {
                        await prisma.hotel.update({
        ...
        if (newPaymentStatus === 'PAID' && updatedBooking.user) {
            try {
                ...
                await EmailService.sendBookingConfirmationWithVoucher(updatedBooking, updatedBooking.user);
                ...
                try {
                    await EmailService.sendBookingConfirmation(updatedBooking, updatedBooking.user);
                } catch (e) {
...
router.post('/verify', async (req: Request, res: Response) => {
    ...
    PaymentAuditService.log(bookingId, 'MANUAL_VERIFY_ATTEMPT', { tranRef }, clientIp);
    ...
    try {
        ...
        PaymentAuditService.log(bookingId, 'MANUAL_VERIFY_RESULT', { status: responseStatus, data }, clientIp);
        ...
        if (responseStatus === 'A') {
            ...
            return res.json({ success: false, status: 'UNPAID', message: data.payment_result?.response_message || 'Payment not authorized' });
        }

    } catch (error: any) {
        console.error('Verify Error:', error);
        PaymentAuditService.log(bookingId, 'MANUAL_VERIFY_ERROR', error.message, clientIp);
