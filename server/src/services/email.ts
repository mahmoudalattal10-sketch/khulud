...
import { generateVoucherPDF } from './voucherService';

// Email Types
type EmailType = 'CONFIRMATION' | 'CANCELLATION' | 'PAYMENT_RECEIPT';
...
export class EmailService {
    ...
    private static async sendEmail(to: string, subject: string, html: string, attachments: any[] = []) {
        try {
     ...
     */
    static async sendCancellation(booking: any, user: any) {
     ...
     */
    static async sendPaymentReceipt(booking: any, user: any) {
     ...
     */
    static async sendBookingConfirmationWithVoucher(booking: any, user: any) {
        ...
        const roomName = booking.room?.name || 'الغرفة';
        ...
        try {
            ...
            const pdfBuffer = await generateVoucherPDF(booking.id);
